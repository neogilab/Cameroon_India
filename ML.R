...
feature_select_Boruta <- function(matrix, title){
  dir.create(paste0("results/", name_ana, "/ML/", title))
  # select number of trees
  model_trees(matrix, title)
  nb_trees <- readline(prompt="Enter Number of trees: ")
  nb_trees <- as.integer(nb_trees) # convert character into integer
...
model_trees <- function(matrix, title){
  ...
  path_model <- paste0("results/", name_ana, "/ML/", title, "/Model_trees_", title, ".pdf")
  plot(RFModel1)
  dev.copy(x11)
...
B_feature_select <- function(nb_trees, matrix, title){
  ...
  path_features <- paste0("results/", name_ana, "/ML/", title, "/SelectedAttributes_", title, ".csv")
...
mtry_selection <- function(Train, nb_trees, title){
  ...
  mtry=tuneRF(Train[,-3],
              ...
              doBest = TRUE, 
              importance = TRUE)
  path_OOB <- paste0("results/", name_ana, "/ML/", title, "/OOB_error_", title, ".pdf")
  dev.copy(x11)
...
my_heatmap <- function(subset, group, sample_name, nb_features, control){
    path_heatmap <- paste("results/", name_ana, "/figures/", sample_name, "_", nb_features, "_heatmap.pdf", sep ="")
...
benchmarking <- function(subset_1, subset_2, subset_3, name_analysis, condition_1, condition_2){
    ...
    path_ROC <- paste("results/", name_ana, "/ML/RF_CD4/", "ROC_", name_analysis, ".pdf", sep ="")
